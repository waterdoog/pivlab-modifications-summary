	...
	if gui.retr('parallel')==0 ||  gui.retr('video_selection_done') == 1
		...
		for i=1:2*frame_skip_serial:num_frames_to_process
			...
			if isempty(cancel)==1 || cancel ~=1
				...
				resultslist{2,result_idx}=y;
				resultslist{3,result_idx}=u;
				resultslist{4,result_idx}=v;
				resultslist{5,result_idx}=typevector;
				resultslist{6,result_idx}=[];
				if get(handles.algorithm_selection,'Value')==3 %dcc
					correlation_map=zeros(size(x));
				end
				correlation_matrices_list{result_idx}=correlation_matrices;
				resultslist{12,result_idx}=correlation_map;
				gui.put('resultslist',resultslist);
				...
				gui.update_progress(result_idx/num_total_pairs*100)
				gui.put('subtr_u', 0);
				gui.put('subtr_v', 0);
